...
//   Original File Path:  /src/CubeCover/CubeCover.h
...
struct CubeCoverOptions {
     ...
     * debugging.
     */
    enum struct ParameterizationType { PT_SEAMLESS, PT_INTEGERGRID, PT_DISCONTINUOUS } parameterizationType;

    /*
     * How singularities in the input frame field are specified.
     * AH_USEPROVIDED instructs core that local assignments between
     ...
     * AH_RECOMPUTE asks core to ignore any provided assignments and to
     ...
     */
    enum struct AssignmentHandling { AH_USEPROVIDED, AH_RECOMPUTE } assignmentHandling;
     ...
     * of the computed parameterization). core will assign each boundary
     ...
     * high noise.
     */
    enum struct BoundaryConditions { BC_FREE, BC_FORCEINTEGER } boundaryConditions;
     ...
     */
    enum struct MIPSolver { MS_GUROBI, MS_COMISO } solver;
 ...
 */
bool cubeCover(const Eigen::MatrixXd& V, const Eigen::MatrixXi& T, const Eigen::MatrixXd& frames,
               const Eigen::MatrixXi& assignments, Eigen::MatrixXd& parameterization);
