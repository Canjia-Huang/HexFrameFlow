...
//   Original File Path:  /src/CubeCover/readMeshFixed.h
...
namespace CubeCover {
     ...
     * @tparam DerivedF
     * @tparam DerivedT
     * @param[in] mesh_file_name
     * @param[out] V vertices
     * @param[out] T tetrahedra
     * @param[out] F facets
     * @return
     */
    ...
    void exportMESH(const std::string filename,
                    const Eigen::PlainObjectBase<DerivedV>& verts,
                    const Eigen::PlainObjectBase<DerivedT>& tets) {
        // Open file stream
        std::ofstream file;
        file.open(filename);
        ...
        for (int i = 0; i < verts.rows(); ++i) {
            ...
            file << "0\n";  // Extra column of zeros as in MATLAB code
        }
        ...
        // Write 'End' to file and close file stream
        file << "End";
        file.close();
    }
