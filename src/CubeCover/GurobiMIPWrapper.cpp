...
//   Original File Path:  /src/CubeCover/GurobiMIPWrapper.cpp
...
namespace CubeCover
{
        ...
        bool verbose)
    {
        try
        {
            int ndofs = A.cols();
            result.resize(ndofs);
            if (A.rows() != ndofs || b.size() != ndofs)
            {
                return false;
            }
            if (constraints.cols() != ndofs + 1)
                return false;
            ...
            model.set(GRB_DoubleParam_MIPGap, tol);
