...
//   Original File Path:  /src/CubeCover/TetMeshConnectivity.h
...
namespace CubeCover {
    ...
    public:
        ...
        */
        ...
        /*
         * Maps a local index on a face (0 <= idx <= 1) to the local index 
         * on the corresponding tet 
         * Returns -1 if there is no such tet (which can only happen 
         * when the face is on the boundary).
         */
        int faceTetIndex(int face, int idx) const { return faceTetIndices(face, idx); }
