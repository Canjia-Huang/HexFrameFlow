...
//   Original File Path:  /src/CubeCover/TetMeshConnectivity.cpp
...
namespace CubeCover
{
    ...
    TetMeshConnectivity::TetMeshConnectivity(const Eigen::MatrixXi& T) : T(T)
    {
        ...
        faceTetIndices.resize(nfaces, 2);
        faceTetIndices.setConstant(-1);
        ...
        for (int i = 0; i < ntets; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                faceTetIndices(faceid, sign) = jp3;
        ...
        for (int i = 0; i < nTets(); i++)
        {
            for(int j =0 ; j < 4; j++) {
                int fid = tetFace(i, j);
                // std::cout << "fid: " << fid << ", nei tets: " << faceTet(fid, 0) << ", " << faceTet(fid, 1) << std::endl; 
            }
        }
